...
public class OpinionAPI {
	private final static String TAG = OpinionAPI.class.getName();
	private static OpinionAPI instance = null;
	private URL url;

	private LibrosRootAPI rootAPI = null;

	private OpinionAPI(Context context) throws IOException,
	...
	public final static OpinionAPI getInstance(Context context)
			throws LibrosAndroidException {
		if (instance == null)
			try {
				instance = new OpinionAPI(context);
			} catch (IOException e) {
	...
	public OpinionCollection getOpiniones() throws LibrosAndroidException {
		System.out.println("llegamos a get opiniones");
		Log.d(TAG, "getOpiniones()");
		OpinionCollection opiniones = new OpinionCollection();

		HttpURLConnection urlConnection = null;
		try {
			urlConnection = (HttpURLConnection) new URL(rootAPI.getLinks()
					.get("opiniones").getTarget()).openConnection();
			urlConnection.setRequestMethod("GET");
			urlConnection.setDoInput(true);
			urlConnection.connect();
		...
		try {
			...
			JSONObject jsonObject = new JSONObject(sb.toString());
			JSONArray jsonLinks = jsonObject.getJSONArray("links");
			parseLinks(jsonLinks, opiniones.getLinks());
			System.out.println("llegamos al parse?");
			opiniones.setNewestTimestamp(jsonObject.getLong("newestTimestamp"));
			opiniones.setOldestTimestamp(jsonObject.getLong("oldestTimestamp"));
			JSONArray jsonOpiniones = jsonObject.getJSONArray("opiniones");
			System.out.println("antes del for");
			for (int i = 0; i < jsonOpiniones.length(); i++) {
				Opinion opinion = new Opinion();
				JSONObject jsonOpinion = jsonOpiniones.getJSONObject(i);
				opinion.setId(jsonOpinion.getString("id"));
				opinion.setUsername(jsonOpinion.getString("username"));
				opinion.setIdLibro(jsonOpinion.getInt("idLibro"));
				//libro.setLastModified(jsonLibro.getLong("lastModified"));
				System.out.println("obtenemos todos los campos");
				jsonLinks = jsonOpinion.getJSONArray("links");
				parseLinks(jsonLinks, opinion.getLinks());
				opiniones.getOpiniones().add(opinion);
			}
		} catch (IOException e) {
			throw new LibrosAndroidException(
	...
	public Opinion getOpinion(String urlOpinion) throws LibrosAndroidException {
		Opinion opinion = new Opinion();
		HttpURLConnection urlConnection = null;
		try {
			URL url = new URL(urlOpinion);
			urlConnection = (HttpURLConnection) url.openConnection();
			urlConnection.setRequestMethod("GET");
			urlConnection.setDoInput(true);
			urlConnection.connect();
			...
			JSONObject jsonOpinion = new JSONObject(sb.toString());
			opinion.setContenido(jsonOpinion.getString("contenido"));
			opinion.setUsername(jsonOpinion.getString("username"));
			opinion.setIdLibro(jsonOpinion.getInt("idLibro"));
			JSONArray jsonLinks = jsonOpinion.getJSONArray("links");
			parseLinks(jsonLinks, opinion.getLinks());
